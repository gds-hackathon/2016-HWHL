...
- (id)initWithResultString:(NSString *)string {
	...
	if (self = [super init]) {
		...
		self.statusMessage = @NSLocalizedString("订单支付失败", comment: "订单支付失败");
        NSDictionary * jsonQuery = (NSDictionary *)[NSJSONSerialization JSONObjectWithData:[string dataUsingEncoding:NSUTF8StringEncoding]
                                                                                   options:0
                                                                                     error:nil];
