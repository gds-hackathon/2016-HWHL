...
- (NSString *)urlEncodedString:(NSData *)src
{
	...
	for(int i = 0;i<len;i++){
		...
		if( ('a' <= c && c <= 'z')
		   || ('A' <= c && c <= 'Z')
		   || ('0' <= c && c <= '9') ){
			NSString* ts = [NSString stringWithCString:(char *)&c encoding:NSUTF8StringEncoding];//[[NSString alloc] initWithCString:(char *)&c length:1];
			...
			char ts1 = hex[c >> 4];
			//	NSLog(@"ts = %c", ts1);
			NSString* ts = [NSString stringWithCString:&ts1 encoding:NSUTF8StringEncoding];//[[NSString alloc] initWithCString:&ts1 length:1];
			[s appendString:ts];
			char ts2 = hex[c & 15];
			ts = [NSString stringWithCString:&ts2 encoding:NSUTF8StringEncoding];//[[NSString alloc] initWithCString:&ts2 length:1];
			[s appendString:ts];
...
//    return result;
	...
	[result appendString:[publicKey stringByReplacingOccurrencesOfString:@" " withString:@"\n"]];
//	int count = 0;
//	
...
//	char *encodedPath = (char *)[_pathForPEMFile cStringUsingEncoding:NSUTF8StringEncoding];
	...
	NSString *temporaryPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
	NSString *path = [temporaryPath stringByAppendingPathComponent:@"AiBa-RSAPublicKey"];
