...
- (NSString *)urlEncodedString:(NSData *)src
{
	...
	for(int i = 0;i<len;i++){
		...
		if( ('a' <= c && c <= 'z')
		   || ('A' <= c && c <= 'Z')
		   || ('0' <= c && c <= '9') ){
			NSString *ts = [NSString stringWithCString:(char *)&c encoding:NSUTF8StringEncoding];
			[s appendString:ts];
		} else {
			[s appendString:@"%"];
			char ts1 = hex[c >> 4];
...
- (NSString *)signString:(NSString *)string {
	...
	NSString *path = [temporaryPath stringByAppendingPathComponent:@"AiBa-RSAPrivateKey"];
...
//		NSData * UTF8Data = [base64String dataUsingEncoding:NSUTF8StringEncoding];
		signedString = [base64String URLEncode];//e [self urlEncodedString:UTF8Data];
    }
